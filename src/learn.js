...
function lmLoadQ() {
  ...
  document.getElementById('lm-card-area').innerHTML = `
    <div class="q-card lm-q-card">
      ...
      ${q.fig ? `<div class="q-figure">${q.fig}${q.figCredit ? `<div class="fig-credit">📷 ${q.figCredit.author} · ${q.figCredit.license}</div>` : ''}</div>` : ''}
    </div>
...
function lmReveal(ok, q) {
  ...
  else { LM.wrong++; recordWeak(q); if (LM.phase === 'learn') LM.wrongQueue.push(q); }
